...
import { freezeFinale, migrateTours, summarizeHistory, type FinaleSnapshot, type TourStats } from './scoring.js';
...
type Tour = { id:string; name:string; seed:number; status:string; townIds:string[]; stopIndex:number; funds:number; reputation:number; inspiration:number; version:number; actors:(Actor & {level:number;xp:number;stamina:number;fatigue:number})[]; visited:string[]; clues:Record<string,string[]>; draft?:Draft; history:any[]; unlocked:string[]; stats?:TourStats; finale?:FinaleSnapshot };
const file = process.env.TOUR_DATA_FILE || join(process.cwd(), 'data.json');
// 加载存档时统一迁移：历史快照不可变，按同一口径回填 stats；已落幕的旧结局冻结不重算
function loadTours():Tour[]{if(!existsSync(file))return [];const migrated=migrateTours(JSON.parse(readFileSync(file,'utf8')));if(migrated.changed)writeFileSync(file,JSON.stringify(migrated.tours,null,2));return migrated.tours as Tour[]}
...
app.post('/api/v1/tours/:id/production/validate',(req,res)=>{const t=getTour(req,res);if(!t||!('id'in t))return;const d=req.body as Draft;const r=validate(t,d);ok(res,{valid:r.errors.length===0,errors:r.errors,preview:{funds:Math.max(0,t.funds-8),inspiration:Math.max(0,t.inspiration-(Array.isArray(d?.endings)&&d.endings.length===3?1:0))}})});
app.put('/api/v1/tours/:id/production',(req,res)=>{const t=getTour(req,res);if(!t||!('id'in t))return;const d=req.body as Draft;const r=validate(t,d);if(r.errors.length)return fail(res,'INVALID_PRODUCTION',r.errors.join('；'),422);t.draft=d;t.status='READY';t.version++;persist();ok(res,{tour:t})});
app.post('/api/v1/tours/:id/performances',(req,res)=>{const t=getTour(req,res);if(!t||!('id'in t))return;const key=String(req.header('Idempotency-Key')||'');const previous=key&&performanceKeys.get(key);if(previous){ok(res,{performance:previous,tour:t,replayed:true});return}if(t.status!=='READY')return fail(res,'INVALID_TOUR_STATE','请先完成合法编排',409);const submitted=req.body as Partial<Draft>|undefined;const d=submitted?.playId?submitted as Draft:t.draft!;const r=validate(t,d);if(r.errors.length)return fail(res,'INVALID_PRODUCTION',r.errors.join('；'),422);const p=r.p!,cur=town(t);const tagScore=d.timeline.reduce((s:number,e:any)=>s+(actions.find(a=>a.id===e.actionId)?.tags.some(x=>cur.mood.includes(x)||p.tags.includes(x))?5:1),0);const score=Math.max(0,Math.min(100,45+tagScore+Math.min(20,t.reputation/5)-d.timeline.length*1));const fill=Math.min(1,0.45+t.reputation/160+tagScore/250);const income=Math.round(cur.capacity*fill*cur.ticket/10);const feedback=score>=75?`${cur.audience}喜欢你对“${cur.legend}”的回应，散场后仍在讨论结尾。`:`观众希望动作更贴近${cur.mood}的气氛；有人记住了你的勇气。`;const perf={id:randomUUID(),idempotencyKey:key||undefined,townId:cur.id,score,income,feedback,ending:d.endings[2],snapshot:d,createdAt:new Date().toISOString()};if(key)performanceKeys.set(key,perf);t.history.push(perf);t.funds+=income;t.reputation=Math.max(0,Math.min(100,t.reputation+Math.round((score-55)/5)));t.inspiration=Math.max(0,t.inspiration+(score>72?1:0));for(const a of t.actors){a.xp+=Math.round(score/4);a.stamina=Math.max(10,a.stamina-12);a.fatigue=Math.min(3,a.fatigue+(score<50?1:0));if(a.xp>=a.level*100){a.level++;a.xp-=a.level*100}}t.visited.push(cur.id);t.stats=summarizeHistory(t.history);t.status=t.stopIndex>=t.townIds.length-1?'FINALE_READY':'ROUTE_SELECTION';t.draft=undefined;t.version++;persist();ok(res,{performance:perf,tour:t})});
app.post('/api/v1/tours/:id/rest',(req,res)=>{const t=getTour(req,res);if(!t||!('id'in t))return;for(const a of t.actors)a.stamina=Math.min(100,a.stamina+25),a.fatigue=Math.max(0,a.fatigue-1);t.version++;persist();ok(res,{tour:t})});
app.get('/api/v1/tours/:id/history',(req,res)=>{const t=getTour(req,res);if(!t||!('id'in t))return;ok(res,{history:t.history,stats:summarizeHistory(t.history)})});
app.post('/api/v1/tours/:id/finale',(req,res)=>{const t=getTour(req,res);if(!t||!('id'in t))return;
  // 已定格的终局（含迁移保留的旧结局）只回放，绝不重算
  if(t.finale){const f=t.finale;ok(res,{ending:f.ending,average:f.averageRounded,tour:t,replayed:true,legacy:!!f.legacy});return}
  if(t.status!=='FINALE_READY')return fail(res,'INVALID_TOUR_STATE','终局尚未开放',409);
  // 与历史快照、迁移档、导出完全同口径：首站计入，单场即该场分数，不按零分
  const f=freezeFinale(t.history,new Date().toISOString());t.finale=f;t.status='COMPLETED';t.version++;persist();
  ok(res,{ending:f.ending,average:f.averageRounded,tour:t})});
// 导出台账：汇总口径与历史快照/终局完全一致，旧结局沿用冻结结果不重算
app.get('/api/v1/tours/:id/export',(req,res)=>{const t=getTour(req,res);if(!t||!('id'in t))return;
  const stats=summarizeHistory(t.history);
  const ledger={schema:'puppet-troupe-tour/export/v2',exportedAt:new Date().toISOString(),tour:{id:t.id,name:t.name,seed:t.seed,status:t.status,version:t.version},stats,finale:t.finale??null,history:t.history};
  res.setHeader('Content-Disposition',`attachment; filename="tour-${t.id}.json"`);ok(res,ledger)});
app.use(express.static(join(process.cwd(),'dist-web')));
// 直接运行时监听；被测试模块导入时不占用端口
const isMain = process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1];
if (isMain) app.listen(3001,()=>console.log('API listening on http://localhost:3001'));
export { app };
